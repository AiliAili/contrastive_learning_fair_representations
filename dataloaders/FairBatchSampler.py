...
class FairBatch(Sampler):
    ...
    def __init__(self, model, x_tensor, y_tensor, z_tensor, batch_size, alpha, target_fairness, replacement = False, seed = 0):
        ...
        self.lb2 = (self.S[self.negative,1])/(self.S[self.negative,1]+(self.S[self.negative,0]))
    ...
    def adjust_lambda(self):
        ...
        if self.fairness_type == 'eqopp':
            ...
            y0_diff = abs(yhat_yz[(self.negative, 1)] - yhat_yz[(self.negative, 0)])
    ...
    def __iter__(self):
        ...
        if self.fairness_type == 'original':
            ...
            if self.fairness_type == 'eqopp':
                ...
                each_size[(self.negative,1)] = round(self.S[(self.negative,1)])
                each_size[(self.negative,0)] = round(self.S[(self.negative,0)])
                ...
                each_size[(1,1)] = round(self.lb1 * (self.S[(1,1)] + self.S[(1,0)]))
                each_size[(1,0)] = round((1-self.lb1) * (self.S[(1,1)] + self.S[(1,0)]))
                each_size[(self.negative,1)] = round(self.lb2 * (self.S[(self.negative,1)] + self.S[(self.negative,0)]))
                each_size[(self.negative,0)] = round((1-self.lb2) * (self.S[(self.negative,1)] + self.S[(self.negative,0)]))
            ...
            sort_index_y_0_z_1 = self.select_batch_replacement(each_size[(self.negative, 1)], self.yz_index[(self.negative,1)], self.batch_num, self.replacement)
            sort_index_y_1_z_0 = self.select_batch_replacement(each_size[(1, 0)], self.yz_index[(1,0)], self.batch_num, self.replacement)
            sort_index_y_0_z_0 = self.select_batch_replacement(each_size[(self.negative, 0)], self.yz_index[(self.negative,0)], self.batch_num, self.replacement)
